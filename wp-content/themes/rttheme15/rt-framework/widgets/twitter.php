...
class Twitter_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ($title) echo $before_title . $title . $after_title;
	 	wp_enqueue_script('jquery-tweet', THEMEURI  . '/js/jquery.tweet.js');

		echo '<div id="'.$randomID.'">';
		echo '<script type="text/javascript">jQuery(window).load(function(){jQuery("#'.$randomID.'").tweet({ count: '.$show_tweets.',count: '.$show_tweets.', username: "'.$twitter_username.'", loading_text: "..." });});</script>';
		echo '</div>'; 
		echo $after_widget;
	}
